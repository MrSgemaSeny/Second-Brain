# MrDevCourses — Обучающая платформа Mr Developer

**Репо:** github.com/MrSgemaSeny/MrDevCourses  
**Деплой:** Fly.io (Backend) + Vercel / GitHub Pages (Frontend)  
**API:** `/api/v1/**`  
**Старт:** 2026-08-25  
**Уровень платформы:** **Level 3 — Strong Educational MVP / Pre-Release Pilot** (100% Green: 236 JUnit backend, 73 Vitest frontend)  
**Фаза:** Фазы 0–5 полностью завершены (B2C Discovery, Curriculum Accordion, Drip Calculation, AI Grader & AST Guard, Hybrid RAG RRF, Quiz Anti-Cheat, PDF Certificates, Admin Suite, Telegram Bot)  
**Линейка курсов платформы (`mr-developer-curriculum.md`):**
- **Курсы Уровня 1 (Базовый)**: Вайбкодинг, AI-ассистенты, Git, FSD, Лендинг + Клиентский Маркетплейс
- **Курс Уровня 2 (ОСНОВНОЙ КУРС / Флагман Mr Developer)**: Full-Stack (Spring Boot + React + PostgreSQL), RBAC, OAuth2, Three.js 3D (Трекер денег), CRM Kanban + Telegram Bot + CI/CD
- **Курсы Уровня 3 (Продвинутый AI SaaS)**: AI Core, Streaming SSE, RAG, WebClient, PII-маскирование, Google SMTP (Pensee)

---

## Содержание (Модули)

- [[01_introduction|1. Введение и обзор проекта]]
- [[02_tech_stack|2. Технологический стек]]
- [[03_architecture|3. Архитектура системы]]
- [[04_system_modules|4. Модули системы (Детальный разбор 10 подсистем)]]
- [[05_roles_and_security|5. Ролевая модель и безопасность (Defense-in-Depth)]]
- [[06_mvp_roadmap|6. Roadmap (Фазы реализации)]]
- [[07_risks_and_pitfalls|7. Риски, подводные камни и извлечённые уроки]]
- [[08_medev_reusability|8. Связи с MeDev (Что переиспользовать)]]
- [[09_course_philosophy_and_mentorship|9. Философия курса и Архитектура Менторства]]
- [[_status|Текущий статус и чек-лист готовности]]

---

## 🔗 Связи в Базе Знаний (Knowledge Graph)
- **Педагогика & Автоматизация:** [[knowledge/pedagogy-and-automation-split-for-vibe-coding|Педагогика вайбкодинга и разделение автоматизации в LMS]]
- **Архитектура & RAG:** [[knowledge/arch-hybrid-rag-dense-sparse-rrf|Гибридный RAG: pgvector HNSW + FTS RRF]], [[wiki/concepts/hybrid-rag-systems|Концепция RAG]]
- **AI & Grader:** [[knowledge/arch-ai-code-grader-and-security-scanner|AI Grader & AST Guard]], [[knowledge/arch-ai-tutor-lesson-grounding|AI-тьютор с Grounding]]
- **Квизы & Оценка:** [[knowledge/arch-quiz-assessment-engine-anti-cheat|Анти-чит движок квизов]]
- **События & Надежность:** [[knowledge/arch-transactional-outbox-event-automation|Transactional Outbox в монолите]], [[knowledge/arch-tiered-rate-limiting-bucket4j|3-Tier Rate Limiting]]
- **Сертификаты:** [[knowledge/arch-pdf-openhtmltopdf-thymeleaf|Векторная генерация PDF (OpenHTMLtoPDF)]]
- **Frontend & B2C:** [[knowledge/b2c-lms-course-discovery-and-video-preview|B2C витрина и Hover-трейлеры]], [[wiki/concepts/fsd-architecture|FSD Архитектура]]
- **ADR Реестр:** [[decisions/ADR-014-b2c-course-catalog-and-detail-experience|ADR-014: B2C витрина и аккордеон]]
- **Глобальный контекст:** [[context/projects|Реестр проектов]], [[context/me|Досье]], [[mem/goals|Цели]]

