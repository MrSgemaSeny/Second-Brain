...
### Фикс: Белый экран на фронтенде (Infinite Redirect Loop)
- **Проблема**: При заходе на /applicant или /company фронтенд зависал и показывал белый экран. Ошибка возникала из-за того, что роли на фронтенде (pplicant, company) не совпадали с ролями, которые возвращал новый бэкенд (CANDIDATE, COMPANY_ADMIN). В результате ProtectedRoute кидал пользователя в бесконечный цикл редиректов.
- **Решение**: В файле rontend/src/app/providers/RouterProvider.tsx заменены старые роли на актуальные (CANDIDATE, COMPANY_ADMIN, ADMIN). Также обновлена проверка роли в VacancyDetailPage.tsx.

### Редизайн: Минималистичный аватар в хедере
- **Изменения**: Убраны синие цвета и круглые элементы (rounded-full) по всему интерфейсу. В хедере громоздкая серая таблетка заменена на аккуратный минималистичный блок (фото с rounded-lg + имя пользователя).
- **Аватар**: Добавлена поддержка user.avatarUrl, временно установлена дефолтная аватарка Ryo-avatar.jpg.
