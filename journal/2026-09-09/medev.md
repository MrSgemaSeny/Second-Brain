...
## Отказоустойчивый парсинг SSE чата и декодер токенов cleanContent

### 1. Первопричина сырого вывода JSON в UI
- При переходе на структурированный SSE-стриминг `{ "content": chunk }` старые сессии в браузере или клиенты без жесткой перезагрузки страницы (Ctrl+F5) получали токены как сырые строки `data:{"content":"..."}`.
- Если старый фронтенд делал `line.substring(5)`, в историю сообщений попадал непрерывный поток сырых JSON-объектов вида `{"content":"###"}{"content":" Что"}`.

### 2. Выполненные исправления
- **`AiChatWidget.tsx` & `useAiGenerate.ts`**:
  - Реализована функция `cleanContent(text)`, которая на лету распаковывает склеенные JSON-токены вида `{"content":"..."}` через безопасное регулярное выражение и `JSON.parse`.
  - В цикле чтения стрима добавлен парсинг склеенных и сырых токенов.
  - В рендеринге компонента `AiChatWidget` отображение сообщений обернуто в `cleanContent(msg.content)`, что гарантирует корректный рендеринг даже для сообщений из кэшированного состояния Zustand.
