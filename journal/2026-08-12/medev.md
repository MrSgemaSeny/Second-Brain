...
- **Масштабный UI редизайн:** Выполнен переход с 3-уровневой навигации на 2-уровневую. `AppHeader.tsx` и `ProfileSidebar.tsx` полностью удалены. Навигация "Main" и "Sections" объединена в единый глобальный компонент `AppSidebar.tsx`. Настройки профиля (`UserProfileDropdown`) перенесены в низ сайдбара с открытием вверх.
- **Очистка стилей и макет AboutSection:** Восстановлены строгие токены GitHub Dark Mode в `index.css`. Компонент `AboutSection.tsx` полностью переписан под новый HTML-макет с использованием 3-колоночной сетки и минималистичных компонентов без применения эффектов Glassmorphism.
