...
11. **Комплексные Сквозные Жизненные Циклы Бизнес-Сущностей (Full Lifecycle E2E)**:
    - Разработаны 5 специализированных боевых E2E-сьютов полного жизненного цикла, централизованный модуль аутентификации с дисковым кешированием `auth-helper.mjs` и мастер-раннер `tests/run-all-e2e.mjs`:
      - **1. CRM Tasks & Pipeline Lifecycle (`tests/e2e/crm-lifecycle-live.mjs`)**: 12 из 12 проверок успешно (100% PASS). Опрос пайплайнов -> верификация стадий -> создание задачи администратором -> чтение деталей -> изменение описания -> перевод задачи на следующую стадию -> назначение сотрудника -> добавление комментария администратором -> чтение комментариев сотрудником -> аудит-история таймлайна -> удаление задачи -> валидация 404.
      - **2. LMS Courses & Learner Progression Lifecycle (`tests/e2e/lms-lifecycle-live.mjs`)**: 11 из 11 проверок успешно (100% PASS). Создание курса администратором -> создание главы -> добавление урока (`DOCUMENT`) -> публикация курса -> обнаружение обучающимся в опубликованном каталоге -> открытие структуры и плана урока -> завершение урока студентом -> запись прогресса -> снятие с публикации администратором -> проверка скрытия из каталога -> удаление независимого черновика.
      - **3. Chat & Real-Time Notifications Lifecycle (`tests/e2e/chat-notifications-live.mjs`)**: 8 из 8 проверок успешно (100% PASS). Список уведомлений -> массовая отметка прочитанности (`read-all`) -> список чат-контактов -> отправка прямого сообщения сотруднику -> фиксация счетчика непрочитанных (`unread > 0`) -> чтение истории диалога сотрудником -> отметка прочитанности диалога -> сброс счетчика непрочитанных в 0.
      - **4. Documents Management & Global Search Lifecycle (`tests/e2e/documents-search-live.mjs`)**: 9 из 9 проверок успешно (100% PASS). Запрос шаблонов документов -> загрузка PDF через multipart/form-data -> проверка появления в общем реестре -> перевод статуса в `REVIEW` -> чтение документа клиентом -> потоковая выгрузка файла (`/download`) с проверкой байтов -> глобальный полнотекстовый поиск по ключевым словам -> удаление файла -> проверка недоступности (404/403).
      - **5. Billing & Invoices Lifecycle (`tests/e2e/billing-invoices-live.mjs`)**: 5 из 5 проверок успешно (100% PASS). Выставление счета клиенту со статусом `ISSUED` -> чтение счетов клиентом -> корректировка суммы и статуса на `PAID` администратором -> удаление счета администратором -> валидация отсутствия в реестре.
    - **Архитектурные выводы и обнаруженные нюансы**:
...
13. **Актуализация Инженерной Документации и Эпиков (Epics & Docs)**:
    - **Инженерная документация (`docs/`)**:
      - `docs/ARCHITECTURE.md`: расширен раздел ролевой модели (строгая Read-Only изоляция `ADVISOR`), документированы три сервиса проверки доступа (`CrmAccessService`, `InvoiceAccessService`, `DocumentAccessService`), каскадная целостность PostgreSQL в LMS и 3-уровневая архитектура автоматизированного тестирования.
      - `docs/RUNBOOK.md`: добавлен сценарий F (ликвидация инцидентов с памятью и алертом `metaspace-near-limit` в Grafana) и регламент запуска верификационных тестов с кешированием токенов.
    - **Синхронизация эпиков (`Epics/Plan/`)**:
      - `Epic-02-crm`: актуализирована история US-02.4 и критерии приемки (роль `ADVISOR` переведена в Read-Only режим, зафиксировано 100% прохождение 12/12 E2E проверок).
      - `Epic-03-documents`: отмечены выполненными все пункты Definition of Done, актуализирована US-03.5 (Read-Only доступ советника без прав удаления/мутаций) и зафиксировано прохождение 9/9 E2E проверок.
      - `Epic-04-lms`: отмечены выполненными все пункты Definition of Done, зафиксирована каскадная очистка зависимостей при удалении курса и явное сохранение глав для гарантии возврата ID, зафиксировано прохождение 11/11 E2E проверок.
      - `Epic-05-chat`: отмечены выполненными все пункты Definition of Done, зафиксировано прохождение 8/8 E2E проверок.
      - `Epic-07-billing`: закрыта история US-07.8 (генерация PDF с отказоустойчивой проверкой шрифтов), зафиксирован новый эндпоинт `GET /{id}` с защитой от IDOR и прохождение 5/5 E2E проверок.
      - `Epic-19-advisor`: зафиксирован статус супервизора (строгий Read-Only без возможности модификации задач и документов).
